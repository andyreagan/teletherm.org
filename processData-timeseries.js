...
var processData = function(error,results) {
    ...
    // var visualization = cityTimeseries();
    // // visualization.plot_all(min_years,flattened_summer_extents,flattened_winter_extents,local_window);
    // visualization.plot_T(min_years,local_window,"summer","red",all_tmax_avg.map(function(d) { return d3.max(d); }),all_tmax_avg_smoothed.map(function(d) { return d3.max(d); }),tmax_raw_good_years,tmax_raw_maxs);
    // visualization.plot_tele(min_years,flattened_summer_extents,all_summer_dates,local_window,"summer");
    // visualization.plot_T(min_years,local_window,"winter","blue",all_tmin_avg.map(function(d) { return d3.min(d); }),all_tmin_avg_smoothed.map(function(d) { return d3.min(d); }),tmin_raw_good_years,tmin_raw_mins);
    // // visualization.plot_tele(min_years,flattened_summer_extents,flattened_winter_extents,local_window);
    // visualization.plot_tele(min_years,flattened_winter_extents,all_winter_dates,local_window,"winter");
    ...
    // store all of these in an array, and then loop through and fix all of the y axes.
    // thinking that the x axes will be fixed for everything!
    // get a linked hover connecting them all
}
