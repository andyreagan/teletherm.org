...
var city_clicked_initial_load = function(d) {
    ...
    // city_name = [proper_city_name.join(" "),city_name_split[1]].join(",");
    city_name = d[3];
    // +" from "+full_year_range[yearIndex]+"&ndash;"+(full_year_range[yearIndex]+parseInt(windowDecoder().cached))+":";

    // document.getElementById("stationname").innerHTML = city_name;
    
    // $("#myinput").attr("value",city_name);
    ...
    // could also set up my own php script if this breaks
