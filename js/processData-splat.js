...
var processData = function(error,results) {
    ...
    end_year = full_year_range[full_year_range.length-1-all_tmax_avg.length+1];
    start_year = end_year-local_window+1;
    var viz1 = citySplat();
    viz1.plot(all_tmax_avg[0],all_summer_dates[0],all_summer_extents[0],all_tmax_avg_smoothed[0],all_tmin_avg[0],all_winter_dates[0],all_winter_extents[0],all_tmin_avg_smoothed[0],"#figure1",start_year+"&ndash;"+end_year);

    end_year = full_year_range[full_year_range.length-1-all_tmax_avg.length+1+20];
    start_year = end_year-local_window+1;
    var viz2 = citySplat();
    viz2.plot(all_tmax_avg[20],all_summer_dates[20],all_summer_extents[20],all_tmax_avg_smoothed[20],all_tmin_avg[20],all_winter_dates[20],all_winter_extents[20],all_tmin_avg_smoothed[20],"#figure2",start_year+"&ndash;"+end_year);

    end_year = full_year_range[full_year_range.length-1-all_tmax_avg.length+1+all_tmax_avg.length-1];
    start_year = end_year-local_window+1;
    var viz3 = citySplat();
    viz3.plot(all_tmax_avg[all_tmax_avg.length-1],all_summer_dates[all_tmax_avg.length-1],all_summer_extents[all_tmax_avg.length-1],all_tmax_avg_smoothed[all_tmax_avg.length-1],all_tmin_avg[all_tmax_avg.length-1],all_winter_dates[all_tmax_avg.length-1],all_winter_extents[all_tmax_avg.length-1],all_tmin_avg_smoothed[all_tmax_avg.length-1],"#figure3",start_year+"&ndash;"+end_year);
}
