var citySplat = function() {
	...
	//     .attr("height", height)
	//     .attr("class", "bg")
	//     .styles({'stroke-width':'0','stroke':'rgb(0,0,0)'})
	//     .attr("fill", "#FCFCFC");
	...
	//     .attr("transform", "rotate(-90.0," + (18) + "," + (figheight/2) + ")");
        ...
        function drawTarget() {
            ...
            axes.append("path")
	        .attrs({ "d": arc({"startAngle": 0, "endAngle": 2*Math.PI}),
                        "class": "arc",
                        "transform": "translate("+width/2+","+height/2+")",
	              })
                .styles({"fill": "rgba(230,230,230,.5)"})
        ...
        function drawLines(temps) {
            ...
            for (var i=0; i<temps.length; i++) {
	            ...
	            .attrs({ "r": y(temps[i]),
                    ...
                    .styles({"stroke": "rgba(210,210,210,1)","fill":"none"});
                var rotation = 3;
                axes.append("text")
	            .attrs({ "transform": function() {
                    ...
                    .styles({"fill": "rgba(160,160,160,1)","align":"left","font-size": "9px"})
        ...
        var angle_round = d3.scaleLinear()
        ...
        var date_text = axes.append("text")
            ...
            .attrs({"x": width-5, "y": 10})
            .styles({"text-anchor": "end", "font-size": "10px", "fill": "rgb(160,160,160)"})
            .text("");
        ...
        var minT_text = axes.selectAll(".minT-text")
            ...
            .attrs({"x": width-5, "y": 30})
            .styles({"text-anchor": "end", "font-size": "10px", "fill": "rgb(160,160,160)"})
            .text("");        

        axes.append("circle")
	    .attrs({ "r": y(linetemps[linetemps.length-1]),
                    "class": "",
                    "cx": width/2,
                    "cy": height/2,
	          })
            .styles({"stroke": "none","fill":"rgba(255, 248, 220, .2)"})
        ...
        function round_ticks() {
            axes.selectAll("line.monthtick").data(month_lengths_cum)
                .enter()
                .append("line")
                .attrs({"x1": function(d,i) { return Math.cos(x_max(d))*y(95)+width/2; },
                ...
                .styles({"stroke": "rgba(210,210,210,1)",
                        "fill": "none"});

            axes.selectAll("defs").data(month_lengths_cum)
                .enter()
                .append("defs").append("path")
                .attrs({"d": function(d,i) { return getPathData(102,d,d+30); },
        ...
        var summer_teletherm_extent_extended = Array();
        console.log(summer_teletherm_extent);
        for (var i=0; i<summer_teletherm_extent.length; i++) {
            summer_teletherm_extent_extended.push([summer_teletherm_extent[i][0]]);
            while ((summer_teletherm_extent_extended[i][summer_teletherm_extent_extended[i].length-1]) < summer_teletherm_extent[i][1]) {
                summer_teletherm_extent_extended[i].push(summer_teletherm_extent_extended[i][summer_teletherm_extent_extended[i].length-1]+1);
            }
        }
	    ...
	    .enter()
	    .append("circle")
	    .attrs({ "cx": function(d,i) { return y(d)*Math.cos(x_max(i+1))+width/2; },
	    ...
	    .enter()
	    .append("circle")
	    .attrs({ "cx": function(d,i) { return y(d)*Math.cos(x_min(i+1))+width/2; },
        ...
        var wintertext = "Winter Teletherm: "+month_names[(month-1-6+12) % 12].slice(0,3)+" "+day+" (day "+winter_teletherm_date+"), "+(winter_teletherm_extent[winter_teletherm_extent.length-1][1]-winter_teletherm_extent[0][0])+" day extent";
        ...
        axes.append("circle")
	    .attrs({ "cx": function(d,i) { return width/2; },
        ...
        //     .enter()
        //         .append("line")
        //         .attrs({"x1": function(d,i) { return Math.cos(x_max(d))*y(97)+width/2; },
    ...
    replot = function() {
	    ...
	    .attrs({ "cx": function(d,i) { return x_max(i+1); },
	    ...
	    .data([].concat(tmin_avg.slice(184),tmin_avg))
            .transition()        
	    .attrs({ "cx": function(d,i) { return x_min(i+1-181); },
		    "cy": function(d,i) { return y(d); },
	          });
        
        axes.select("line.summerteleline")
            .transition()
	    .attrs({ "x1": x_max(summer_teletherm_date+1),
        ...
        axes.append("line.winterteleline")
            .transition()
	    .attrs({ "x1": x_min(winter_teletherm_date+1),
